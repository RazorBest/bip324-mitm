...
fn main() {
    // Run commands
    // iptables -A INPUT -t mangle -p tcp --dport 8333 -j NFQUEUE --queue-num <YOUR_QUEUE_ID> --queue-bypass
    // iptables -A INPUT -t mangle -p tcp --sport 8333 -j NFQUEUE --queue-num <YOUR_QUEUE_ID> --queue-bypass
    // iptables -A OUTPUT -t mangle -p tcp --dport 8333 -j NFQUEUE --queue-num <YOUR_QUEUE_ID> --queue-bypass
    // iptables -A OUTPUT -t mangle -p tcp --sport 8333 -j NFQUEUE --queue-num <YOUR_QUEUE_ID> --queue-bypass
    //
    // Example: iptables -i eth0 -p tcp -j NFQUEUE --queue-num 0 --queue-bypass
