...
pub struct CipherSession {
    ...
    pub inbound: Option<InboundCipher>,
    /// Encrypts outbound packets.
    pub outbound: Option<OutboundCipher>,
...
mod tests {
    ...
    fn test_decrypt_in_place() {
        ...
        let (mut alice_inbound, mut alice_outbound) = alice_cipher.into_split();
        let (mut bob_inbound, mut bob_outbound) = bob_cipher.into_split();
        ...
        let (packet_type, plaintext) = bob_inbound.decrypt_in_place(&mut ciphertext, None).unwrap();
        ...
        let (packet_type2, plaintext2) = alice_inbound
    ...
    fn test_decrypt_min_length() {
        ...
        .unwrap();
        let alice_cipher = CipherSession::new(session_keys, Role::Initiator);
        let (mut alice_inbound, _alice_outbound) = alice_cipher.into_split();
        ...
        let result = alice_inbound.decrypt(&too_small, &mut plaintext_buffer, None);
        ...
        let result = alice_inbound.decrypt_in_place(&mut too_small, None);
        assert_eq!(result, Err(Error::CiphertextTooSmall));
    }
    ...
    fn test_decrypt_to_vec_packet_too_big() {
        ...
        let result = alice_inbound.decrypt_to_vec(&large_ciphertext, None);
    ...
    fn test_fuzz_packets() {
        ...
        for _ in 0..(224 + 100) {
            ...
            let alice_message_len =
                bob_inbound.decrypt_packet_len(enc_packet[..NUM_LENGTH_BYTES].try_into().unwrap());
            ...
            let bob_message_len = alice_inbound
    ...
    fn test_additional_authenticated_data() {
        ...
        let alice_cipher = CipherSession::new(session_keys.clone(), Role::Initiator);
        let bob_cipher = CipherSession::new(session_keys, Role::Responder);
        let (_alice_inbound, mut alice_outbound) = alice_cipher.into_split();
        let (mut bob_inbound, _bob_outbound) = bob_cipher.into_split();
    ...
    fn test_vector_1() {
        ...
        let alice_message_len =
            bob_inbound.decrypt_packet_len(enc[..NUM_LENGTH_BYTES].try_into().unwrap());
        let mut dec_packet = vec![0u8; InboundCipher::decryption_buffer_len(alice_message_len)];
        bob_inbound
            .decrypt(
    ...
    fn test_vector_2() {
        ...
        let alice_cipher = CipherSession::new(session_keys.clone(), Role::Responder);
        let (_inbound, mut outbound) = alice_cipher.into_split();
